...
class face_emotion():
    ...
    def __init__(self):
        ...
        self.predictor = dlib.shape_predictor("F:\\wink-test\\blink_detect\\model\\shape_predictor_68_face_landmarks.dat")
        # ip 摄像头的 rtsp地址
            ...
            if (len(faces) != 0):
                ...
                for i in range(len(faces)):
                    ...
                    for k, d in enumerate(faces):
                        ...
                        # left、top、right、bottom分别是人脸矩形框的左上角的x、y坐标和右下角的x和y坐标
                        ...
                        for j in range(17, 21):#j为17到21这个序列的遍历
                            brow_sum += (shape.part(j).y - d.top()) + (shape.part(j + 5).y - d.top())
                            frown_sum += shape.part(j + 5).x - shape.part(j).x
                            line_brow_x.append(shape.part(j).x)#在列表末尾添加新对象
                            line_brow_y.append(shape.part(j).y)#在列表末尾添加新对象
